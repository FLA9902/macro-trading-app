...
def get_quandl_data(code):
    ...
    try:
        ...
        st.warning(f"⚠️ Could not fetch {code}. Using fallback. Details: {str(e)}")
        return None

def get_macro_signals():
    cpi_index = get_quandl_data("FRED/CPIAUCSL")  # US CPI Index (not YoY)
    fed_rate = 5.25  # Static
    gdp = 2.1        # Static
    pmi = 52.3       # Static

    score = 0
    score += 1 if cpi_index and cpi_index < 310 else -1
    ...
    return cpi_index, fed_rate, gdp, pmi, score, signal
...
st.metric("US CPI Index (FRED)", round(cpi, 2) if cpi else "Unavailable")
